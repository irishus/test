...
std::pair<int, std::string> ExecuteCommandGDB(const std::string &target_exe) {
  std::string result = "";
	int return_code = 0;
  bool timeout = false;
  // std::stringstream ss;
  ...
  const std::string & temp_file = "temp.txt";
	int outfile = open(temp_file.c_str(), O_CREAT|O_WRONLY|O_TRUNC, 0644);
	int child_status;
	pid_t child_pid = fork();
	if (child_pid < 0){
		close(outfile);
    // Logger::Error("[ExecuteCommandGDB] A fork error has in ExecuteCommandGDB!");
    std::cerr << "[ExecuteCommandGDB] A fork error has occured in ExecuteCommandGDB!\n";
		exit(-1);
	}else if (child_pid == 0) {
		// child
		dup2(outfile, 1); // replace stdout
		close(outfile);
		// return_code = execlp("gdb", "gdb", "--batch", "--command=test.gdb", "--args", target_exe.c_str());
		return_code = execlp("gdb", "gdb", "--batch", "--command=test.gdb", "--args", target_exe.c_str(), "2>&1");
	}else {
		// parent
		int w;
		int cnt = 0;
		int timeout_ms = 5000000; // 5 seconds
		int poll_ms = 500000;
		do {
          ...
          // ss << "[ExecuteCommandGDB] GDB Command exited by signal " << return_code;
          // Logger::Info(ss.str());
					break;
				}
			}
		...
		if (cnt >= (timeout_ms / poll_ms)) {
      // ss << "[ExecuteCommandGDB] Timeout reached!";
      // Logger::Info(ss.str());
      timeout = true;
  ...
  // TODO hanlde result output if timeout
	// read the temp.txt and remove it
	if (std::ifstream temp{temp_file}) {
...
CoverageReport ParseFromLCOVOutput(const std::string &output) {
  ...
  // THE BELOW ASSERT SOMETIME FAIL WHEN GET INTERRUPT
  assert(counter == 3);
...
const std::string &CoverageObserver::GetCleanCovCommand() const {
  return clean_cov_command_;
}
const std::string &CoverageObserver::GetCheckGCNOCommand() const {
  return check_gcno_command_;
}
const std::string &CoverageObserver::GetMeasureCovCommand() const {
  return measure_cov_command_;
}
...
CoverageReport CoverageObserver::MeasureCoverage() {
  const std::string &command = GetMeasureCovCommand();
...
void CoverageObserver::CleanCovInfo() {
  const std::string &command = GetCleanCovCommand();
  ExecuteCommand(command);
  prev_success_ = CoverageReport();
}
bool CoverageObserver::IsGCNOFileExisted() {
  const std::string &command = GetCheckGCNOCommand();
...
void CoverageObserver::DumpCommand(const std::string &output_dir){
  std::experimental::filesystem::path as_fs_path(output_dir);
  const std::string &filename = "coverage_command";
  const std::experimental::filesystem::path &name = as_fs_path / filename;
  const std::string & name_string = name.string();
  if (std::ofstream target{name_string}) {
    target << GetCleanCovCommand() << "\n";
    target << GetMeasureCovCommand() << "\n";
  }else {
    Logger::Error("[CoverageObserver::DumpCommand]", "Problematic output file: " + name_string + '\n');
  }
}
...
{
  clean_cov_command_ = "find " + object_files_dir_ + R"( -name "*.gcda" -exec rm -f {} \;)";
  check_gcno_command_ = "find " + object_files_dir_ + R"( -name "*.gcno")";
  switch (measurement_tool_) {
    case CoverageMeasurementTool::kGCOVR: {
      ...
      measure_cov_command_ = "gcovr -r " + source_files_dir_ + " -f " + source_files_dir_
    ...
    case CoverageMeasurementTool::kLCOVFILT: {
      ...
      const char *ignore_empty_unused = " --ignore-errors empty,unused";
      ...
      const std::string &cmd2 =
        tool + ignore_empty_unused + ' ' + additional_flags + ' ' + lcov_branch_cov + " -o " + filename2
          + " -r " + filename1 + " '/usr/include/*' '/usr/lib/*'" + suppress_stderr;
      measure_cov_command_ = cmd1 + " && " + cmd2;
      break;
    }
  }
}
...
void CrashTCHandler::WriteGDBCommandFile() {
  // write in working directory is better i think
...
TCMemo CrashTCHandler::ExecuteInGDBEnv(const std::string &target_exe, const std::string &src_dir) {
  ...
  // const std::pair<int, std::string> &execution_result = ExecuteCommand(ss.str()); // previous way of executing gdb command
  const std::pair<int, std::string> &execution_result = ExecuteCommandGDB(target_exe); 
