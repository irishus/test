...
int CLIParsedArgs::GetLibfuzzTimeoutInSeconds() const {
  return libfuzz_timeout_in_seconds_;
}
void CLIParsedArgs::SetLibfuzzTimeoutInSeconds(int libfuzz_timeout) {
  libfuzz_timeout_in_seconds_ = libfuzz_timeout;
}
...
static llvm::cl::opt<int> kOptLibfuzzerTimeout(
  "libfuzz-timeout",
  llvm::cl::desc(
    "Specify libfuzzer timeout (in seconds). Default = 60"),
  llvm::cl::value_desc("int"),
  llvm::cl::init(60),
  llvm::cl::cat(kCxxfoozzOptions));
...
CLIParsedArgs CLIArgumentParser::ParseProgramOpt() {
  ...
  result.SetLibfuzzTimeoutInSeconds(kOptLibfuzzerTimeout.getValue());
