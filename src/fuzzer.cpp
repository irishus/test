...
void PartitionByTimestamp(
  ...
  for (const auto conf : configs) {
    ...
    std::experimental::filesystem::copy(CLEMENTINE_SOURCE_DIR + "/scripts/batch_libfuzzer.py", wd_subfolder + "/batch_libfuzzer.py");
  }
  /* END */
}
...
//  std::string suffix = '_' + std::to_string(idx) + ext;
  ...
  std::experimental::filesystem::copy(CLEMENTINE_SOURCE_DIR + "/scripts/batch_libfuzzer.py", wd_replay + "/batch_libfuzzer.py");
  ...
  assert(!Operand::IsKLibFuzzerMode());
  {
    ...
    std::experimental::filesystem::copy(CLEMENTINE_SOURCE_DIR + "/scripts/batch_libfuzzer.py", wd_libfuzzer + "/batch_libfuzzer.py");
...
//    ImportWriter::ExtractImportWriterFromSourceLoc(compiler_instance, target_class_type->GetModel());
  ...
  // const std::string &output_dir = out_prefix; // [CLION] JUST FOR DEBUGGING IN CLION
...
//          Logger::Warn("Found incompilable test case with ID = " + std::to_string(ftc.GetId()));
  ...
  FlushQueue(
    ...
    program_ctx,
    target_filename,
    parsed_args.GetLibfuzzTimeoutInSeconds()
  );
  // write how to clean the coverage and measure the coverage
  observer.DumpCommand(
    output_dir
  );
}
