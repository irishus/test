...
class CoverageObserver {
 public:
  ...
  const std::string &GetCleanCovCommand() const;
  const std::string &GetCheckGCNOCommand() const;
  const std::string &GetMeasureCovCommand() const;
  ExecutionResult ExecuteAndMeasureCov(const std::string &target_exe);
  CoverageReport MeasureCoverage();
  void CleanCovInfo();
  bool IsGCNOFileExisted();
  void DumpCommand(const std::string &output_dir);
 private:
  ...
  std::string clean_cov_command_;
  std::string check_gcno_command_;
  std::string measure_cov_command_;
